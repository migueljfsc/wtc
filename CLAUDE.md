...
- GitOps engines: the operator runs **Flux only**. Argo CD ingest (P11, the vendor-neutrality proof) is **fixture-supported, not operator-stack** — validated against a kind-local Argo v3.4.5, wired via the canonical template in `docs/setup/argocd-notifications.yaml`. Env inference for argocd never uses cluster=env (its destServer is a URL; one Argo manages many clusters).
- Network: the **operator's own** wtc deploys inside the org's private environment (EKS or similar) with no public endpoint → for them the **GitHub API poller is the primary GitHub ingest path**. But (operator decision 2026-07-16) the **product is designed as if wtc may be reachable from anywhere** — exposure is a per-installation choice, webhook and poller are peer first-class ingest modes (P13), and SaaS webhook senders are in scope (P14 mapping webhook). Flux notification traffic is in-cluster and unaffected either way.
...
- **Auth:** per-source HMAC on webhook paths (GitHub `X-Hub-Signature-256`; Flux `generic-hmac` provider). Exception (operator-approved 2026-07-16): `/ingest/argocd` uses a static `X-WTC-Token` shared secret, constant-time compared — Argo's notification templates cannot HMAC-sign bodies. Static bearer tokens for `/ingest/generic` and all `/api/*`. No users, no RBAC in v1.
...
10. **Argo notifications gate on an annotation hash, both ways.** The notifications-controller's `notified.notifications.argoproj.io` annotation (trigger+condition hash) re-fires on every observed state flip — spam, handled by `sources.argocd.suppression_window` — but also sends NOTHING for a same-revision resync whose transitions the controller never observed. wtc can legitimately miss such a resync; no poller-analog exists for Argo in v1. Row keys are per sync operation (`argocd:<app>:<revision>:<startedAt>`) so a failed→succeeded retry of the same revision is two truthful rows, never a stuck `failed`.
