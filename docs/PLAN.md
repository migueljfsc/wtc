...
| **P21 awareness (outbound)** | ✅ 2026-07-19 | `notifications[]` subscriptions (rules-dialect globs over env/service/repo/kind/status → sink) firing on new rows **and status-changing upserts** — transition detection fell out of the rank guard (an applied UPDATE arm ⇒ strict rank increase ⇒ status changed; suppressed conflict ⇒ no notify), so (event id, status) idempotency needs no notify-side state; upsert RETURNING extended to the post-merge row so matching sees what the ledger kept. Sinks slack/webhook/**grafana-annotation** (round-trip captured live vs Grafana 11.3.0, `testdata/grafana/`); dedicated bounded queue fed from the Ingest funnel via `Store.SetNotifyFunc` (never the SSE broadcaster), 4 attempts + backoff then drop, `wtc_notify_{sent,failed,dropped}_total`; sink secrets masked in config view. Atom `/feed` (?token= auth, entry id = id+status). Durable outbox = stretch, not built |
| **P22 harden the record** | ✅ 2026-07-19 | `wtc export` (csv stable-order / ndjson full / json, streaming page-by-page), `wtc backup` (`VACUUM INTO` on the read pool streamed over `/api/backup`, atomic client write; postgres → 501 + pg_dump docs), `wtc explain` (first-writer-wins trace from replaying CURRENT rules over recorded facts; drift vs stored row flagged). `facts` column (sqlite 0006/pg 0005) captured in `Engine.Apply` — facts + pre-rules preset, redacted; pre-migration & generic rows report "facts not recorded". `docs/setup/{backup,export}.md` + onboarding explain section |
