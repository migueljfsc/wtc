...
func TestDORA(t *testing.T) {
	...
	r, err := DORA(context.Background(), st, base.Add(-time.Hour), base.Add(4*time.Hour), time.Hour)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDORAWindowBoundary(t *testing.T) {
	...
	r, err := DORA(context.Background(), st, base.Add(-time.Hour), base.Add(3*time.Hour), time.Hour)
	if err != nil {
		t.Fatal(err)
	}
